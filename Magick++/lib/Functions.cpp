...
{
  return GetGeometry( geometry_.c_str(), &x_, &y_,
		      &width_, &height_ );
}
...
int Magick::ParseImageGeometry( const std::string &geometry_, int &x_, int &y_,
				unsigned int &width_, unsigned int &height_ )
{
  return ParseImageGeometry( geometry_.c_str(), &x_, &y_,
			     &width_, &height_ );
}

std::string Magick::PostscriptGeometry( const std::string &page_ )
{
  char *pageptr = PostscriptGeometry( page_.c_str() );
