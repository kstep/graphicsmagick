...
#define CompositeCoverage(alpha,beta) ((alpha)+(beta)-(alpha)*(beta)/MaxRGB)
#define CompositeOver(A,alpha,B,beta) ((unsigned long) (MaxRGB*((alpha)*(A)/ \
  MaxRGB+(beta)*((B)-(alpha)*(A)/MaxRGB)/MaxRGB)/CompositeCoverage(alpha,beta)))
