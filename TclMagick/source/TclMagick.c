...
#define TCLMAGICK_VERSION_STR "0.1"
#define TCLMAGICK_VERSION_HI  0
#define TCLMAGICK_VERSION_LO  1
...
typedef struct {
    int             type;
    void            *wandPtr;   /* MagickWand, DrawingWand or PixelWand pointer */
    Tcl_Command     magickCmd;  /* Token for magick command, used to delete it */
    Tcl_Interp      *interp;    /* Tcl interpreter owing the object */
    Tcl_HashEntry   *hashPtr;	/* Hash entry for this structure, used to delete it */
} TclMagickObj;
...
static TclMagickObj *newMagickObj(Tcl_Interp  *interp, int type, void *wandPtr, char *name)
{
     ...
     */
    if( name == NULL ) {
        sprintf(idString, "magick%lX", (int)wandPtr );
        name = idString;
    }
    ...
    Tcl_Obj     *CONST objv[] )     /* List of arguments */
{
    ...
    case TM_TYPE:    /* magick type name */
        { 
            ...
            Tcl_SetResult(interp, objTypeNames[mPtr->type], TCL_VOLATILE);
    ...
    Tcl_Obj     *CONST objv[] )     /* List of arguments */
{
    ...
    if( sizeof(subCmds)/sizeof(subCmds[0]) != TM_END_OF_TABLE + 1 ) {
        Tcl_Panic("magickObjCmd: Invalid subcommand table");
    }
#endif
    ...
    case TM_GAMMA:       /* gamma ?level=1.0? */
    case TM_GAMMA_IMAGE: /* GammaImage ?level=1.0? */
        {
	    double level;

            if( objc > 3) {
                Tcl_WrongNumArgs(interp, 2, objv, "?level=1.0?");
                return TCL_ERROR;
            }
            if( (objc > 2) && ((stat = Tcl_GetDoubleFromObj(interp, objv[2], &level)) != TCL_OK) ) {
                return stat;
            }
            if( objc != 3 ) {
                Tcl_WrongNumArgs(interp, 2, objv, "?level=1.0?");
            ...
            result = MagickGammaImage(wandPtr, level);
            if (!result) {
                return myMagickError(interp, wandPtr);
            }
            break;
    ...
    case TM_GET_NUMBER_OF_IMAGES:  /* GetNumberOfImages */
        {
            ...
            result = MagickGetNumberOfImages(wandPtr);
    ...
    case TM_SET_IMAGE: /* SetImage num */
        {
            ...
            if( (stat = Tcl_GetIntFromObj(interp, objv[2], &num)) != TCL_OK ) {
                 return stat;
            }
            result = MagickSetImage(wandPtr, num);
            Tcl_SetObjResult(interp, Tcl_NewIntObj(result));
            break;
        }
    ...
    case TM_TRANSPARENT_IMAGE:  /* TransparentImage targetPix ?opacity? ?fuzz=0.0? */
	    ...
	    PixelWand *targetPtr;
            ...
            result = MagickTransparentImage(wandPtr, targetPtr, opacity, fuzz);
            if (!result) {
                return myMagickError(interp, wandPtr);
            }
            break;
    ...
    Tcl_Obj     *CONST objv[] )     /* List of arguments */
{
    ...
    static char  *colors[]  = {"blue", "green", "red", "opacity", (char *) NULL};
     ...
     */
    if( sizeof(subCmds)/sizeof(subCmds[0]) != TM_END_OF_TABLE + 1 ) {
        Tcl_Panic("pixelObjCmd: Invalid subcommand table");
    }
#endif
    ...
    Tcl_Obj     *CONST objv[] )     /* List of arguments */
{
    ...
    enum subIndex {
        ...
        TM_SKEWX,           TM_SKEW_X,
        TM_SKEWY,           TM_SKEW_Y,
        TM_STROKECOLOR,     TM_GET_STROKECOLOR,         TM_SET_STROKECOLOR,
        ...
        TM_TEXTUNDERCOLOR,  TM_GET_TEXTUNDERCOLOR,      TM_SET_TEXTUNDERCOLOR,
        TM_TRANSLATE,       TM_TTRANSLATE,
        TM_VIEWBOX,         TM_SET_VIEWBOX,
        TM_END_OF_TABLE
    };
    static char *pushCmdNames[] = {
            ...
            if (objc == 3) { /* Set rule */
                ...
                for (ruleIdx = 0; ruleIdx < sizeof(fillRules)/sizeof(fillRules[0]); ruleIdx++) {
                    if( fillRules[ruleIdx] == rule ) {
                        Tcl_SetResult(interp, fillRuleNames[ruleIdx], TCL_VOLATILE);
                        return TCL_OK;
                    }
                }
            ...
            if (objc == 3) { /* Set rule */
                ...
                for (unitIdx = 0; unitIdx < sizeof(unitTypes)/sizeof(unitTypes[0]); unitIdx++) {
                    if( unitTypes[unitIdx] == unit ) {
                        Tcl_SetResult(interp, unitNames[unitIdx], TCL_VOLATILE);
                        return TCL_OK;
                    }
                }
    ...
    case TM_STROKECOLOR:            /* strokecolor get|set pixel */
    case TM_TEXTUNDERCOLOR:         /* textundercolor get|set pixel */
        {
    ...
    case TM_GET_STROKECOLOR:        /* GetStrokeColor pixel */
    case TM_GET_TEXTUNDERCOLOR:     /* GetTextUnderColor pixel */
       {
            PixelWand   *pixelPtr;
            char        *name;
		...
		return TCL_ERROR;
            ...
            switch ((enum subIndex)index) {
                case TM_GET_FILLCOLOR:
                    ...
                    break;
                case TM_GET_STROKECOLOR:
                    DrawGetStrokeColor(wandPtr, pixelPtr);
                    break;
                case TM_GET_TEXTUNDERCOLOR:
    ...
    case TM_SET_STROKECOLOR:        /* SetStrokeColor pixel */
    case TM_SET_TEXTUNDERCOLOR:     /* SetTextUnderColor pixel */
       {
            PixelWand   *pixelPtr;
            char        *name;
		...
		return TCL_ERROR;
            ...
            switch ((enum subIndex)index) {
                case TM_SET_FILLCOLOR:
                    ...
                    break;
                case TM_SET_STROKECOLOR:
                    DrawSetStrokeColor(wandPtr, pixelPtr);
                    break;
                case TM_SET_TEXTUNDERCOLOR:
            ...
            if (objc == 3) { /* Set font stretch */
                ...
                for (stretchIdx = 0; stretchIdx < sizeof(stretchTypes)/sizeof(stretchTypes[0]); stretchIdx++) {
                    if( stretchTypes[stretchIdx] == stretch ) {
                        Tcl_SetResult(interp, stretchNames[stretchIdx], TCL_VOLATILE);
                        return TCL_OK;
                    }
                }
            ...
            if (objc == 3) { /* Set font style */
                ...
                for (styleIdx = 0; styleIdx < sizeof(styleTypes)/sizeof(styleTypes[0]); styleIdx++) {
                    if( styleTypes[styleIdx] == style ) {
                        Tcl_SetResult(interp, styleNames[styleIdx], TCL_VOLATILE);
                        return TCL_OK;
                    }
                }
            ...
            if (objc == 3) { /* Set gravity */
                ...
                for (gravIdx = 0; gravIdx < sizeof(gravTypes)/sizeof(gravTypes[0]); gravIdx++) {
                    if( gravTypes[gravIdx] == grav ) {
                        Tcl_SetResult(interp, gravNames[gravIdx], TCL_VOLATILE);
                        return TCL_OK;
                    }
                }
            ...
            if (objc == 3) { /* Set font style */
                ...
                for (capIdx = 0; capIdx < sizeof(capTypes)/sizeof(capTypes[0]); capIdx++) {
                    if( capTypes[capIdx] == cap ) {
                        Tcl_SetResult(interp, capNames[capIdx], TCL_VOLATILE);
                        return TCL_OK;
                    }
                }
            ...
            if (objc == 3) { /* Set font style */
                ...
                for (joinIdx = 0; joinIdx < sizeof(joinTypes)/sizeof(joinTypes[0]); joinIdx++) {
                    if( joinTypes[joinIdx] == join ) {
                        Tcl_SetResult(interp, joinNames[joinIdx], TCL_VOLATILE);
                        return TCL_OK;
                    }
                }
            ...
            if (objc == 3) { /* Set antialias flag */
                ...
                for (decoIdx = 0; decoIdx < sizeof(decoTypes)/sizeof(decoTypes[0]); decoIdx++) {
                    if( decoTypes[decoIdx] == deco ) {
                        Tcl_SetResult(interp, decoNames[decoIdx], TCL_VOLATILE);
                        return TCL_OK;
                    }
                }
...
/*
  Initialization procedure for static initialization.
*/
extern int Magick_AppInit(Tcl_Interp *interp);
int Magick_AppInit(Tcl_Interp *interp)
{
