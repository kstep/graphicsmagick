...
SCODE t_stream(BOOL fTestStorage, BOOL fCreate=1)
{
    ...
    {   // some tests that changes the contents
        ...
        assert( 0 == pstmC->Release());
    }

    if (pstm) assert(0 == pstm->Release());
    if (pstm1) assert(0 == pstm1->Release());
    if (pstm2) assert(0 == pstm2->Release());
    if (pstg) assert( 0 == pstg->Release());
    if (pstg1) assert( 0 == pstg1->Release());
    if (pstg2) assert( 0 == pstg2->Release());
    if (pstg3) assert( 0 == pstg3->Release());
